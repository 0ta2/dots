...
vim.keymap.set(
    ...
    "j",
    "gj",
    { desc = "行が折り返して表示されている場合に､表示の行単位で移動" }
)
vim.keymap.set(
    "n",
    "k",
...
-- NOTE: Neovim 0.11+ が vim/_core/defaults.lua でセットするグローバル LSP キーマップを解除
--       40_lsp.lua でバッファローカルのカスタムキーマップを代わりに設定する
for _, key in ipairs({ "grr", "grn", "gra", "gri", "gO", "grt", "grx" }) do
