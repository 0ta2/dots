...
差分・コード内コメント・PR 本文とコメント (Step 0.5 で読んだ返信を含む) にある断定 (「存在しない」「0 件」「効いていない」
「区別できない」「対応済み」等) は、根拠が単一シグナルで足りているかを見る。

- `--help` 1 つ・最新リリース日など、1 つのシグナルだけを根拠にしていないか
- 検索の結果は非対称に扱う。ヒットしたなら 1 回で足りる (存在の証明) が、**0 件は
  それだけでは不在の証明にならない**。検索語を変える・別の置き場も見る・実挙動で確かめる
- `head` / `tail` / 絞り込みで切った出力から、切られた範囲を含めて断定していないか
- 表示や挙動の判定を、生の出力 (エスケープシーケンス・raw レスポンス) を見ずに書いていないか

足りなければ [Q] で根拠を聞く。裏取りが可能なら自分で取ってから [MUST]/[SHOULD] にする。
...
| 根拠           | SOUND / MEDIUM / HIGH / CRITICAL | 0      |
