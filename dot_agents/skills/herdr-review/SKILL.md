...
**ユーザーが種別を指定していないときは、種別を除いた `review-<リポジトリ名>-<識別子>-` の
前方一致で探す。** 既定の種別で組み立てたラベルで探すと、前回そうでない種別に交代していた場合に
既定の種別の古いタブへ戻ってしまう (「前回と同じ担当者に送る」を破る)。ヒットしたタブの種別が
その作業の担当者なので、既定は当てずにそれを使う。ヒットが無いときだけ既定の種別で新しく立てる。
