...
最終レビューとして報告してしまう。マーカーの有無で「相手のターンが終わったか」を
機械的に確認する。
...
レビューが始まる前に通ってしまう。`recent-unwrapped` はメッセージ境界を持たないので
...
残る誤検知は「最後に REVIEW_DONE を出力します」のような相手の実況だけ。
マーカーが回答の**最後の行**として単独で出ていることも併せて見る。実況と
区別がつかない状態が続くなら「回答を tmp に md で書き、最後の行にマーカーを
...
マーカーはあるが回答全文が出ないなら同じくファイル出力を頼み直す。最初の依頼文で
ファイル出力を求めるのは避ける (画面から取れるなら不要)。

**マーカーは「相手のターンが終わった」ことしか保証しない。** herdr は画面から
状態を推定しているだけで、バックグラウンド実行中を idle と誤判定する既知の不具合が
あるため、まずマーカーでターンの終了を機械的に確認する。そのうえで、依頼した対象・
観点をレビューが実際に網羅しているかは本文を読んで自分で判断する。マーカーと
状態表示だけで最終レビューとして扱わない。
...
- 完了マーカーの無い出力を最終レビューとしてそのまま報告しない
