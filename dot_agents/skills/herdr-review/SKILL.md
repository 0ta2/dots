...
- 同じ PR の再レビューなら、**前回と同じエージェント種別に送る** (1 周目を claude で回したなら
  2 周目も claude)。レビュースキルはラウンド番号を投稿本文の識別行から判定するため
  (`change-review` の Step 0.5)、種別が変わると 1 周目に戻る。ラウンド番号を依頼元から渡す必要は
  ない。**モデルだけ変えるのは同じ系列の続き**でラウンドは継続する (識別行のエージェント名は
  変わらない)。種別を変えて 1 周目から読み直させるのは、それを狙うときだけにする
- 除外事項 (別 PR へ分離済み・accepted risk・technical debt) は、**PR が対象なら**依頼文に書かず
  PR 本文の Scope Lock に置く。依頼文で毎回組み立て直すと、ラウンドをまたいで前提がブレる。
  この skill に `gh` は無いので、Scope Lock を書けないだけでなく有無も確認できない。
  除外事項があるとユーザーが言ったときに、PR 本文へ置くよう促す。
  PR 以外 (ローカル差分・特定ファイル) が対象のときは置き場が無いので依頼文に書く
