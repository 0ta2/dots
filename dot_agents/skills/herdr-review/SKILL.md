...
| ヒット | ユーザーの指定 | すること |
| --- | --- | --- |
| 0 件 | — | 新しく立てる (種別は指定があればそれ、無ければ既定) |
| 1 件 | 無し、または同じ種別 | そのまま再利用する (前回と同じ担当者) |
| 1 件 | 違う種別 / 新しい担当者 / 別モデル | 「担当者を交代するとき」へ |
| 2 件以上 | — | 想定外。再利用せずユーザーに報告し、どれに送るか確認する |
...
タブが 2 つ以上ヒットするのは、交代のときに前任者のタブを閉じられなかった場合 (所有権を
証明できずユーザーに確認した場合) に起きる。どちらが現在の担当者かは自分では決めない。
