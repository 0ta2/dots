...
- 対象が PR なら、`change-review` の Step 4 に従って**結果を PR に投稿するのは reviewer 自身の
  仕事**だと明記する。「投稿しないでください」と書かない。依頼元が代理投稿すると、GitHub 上の
  投稿者名はどちらも同じアカウントなので、本文の識別行だけが誰の指摘かを示す手掛かりになる。
  reviewer 自身に書かせるほうがずれない
