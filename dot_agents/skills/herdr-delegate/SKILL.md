...
- 最後に何を報告してほしいか (下記)
- 報告の最後に単独行で `IMPLEMENT_DONE` と出力すること (他の文字と同じ行に混ぜない)
...
**`IMPLEMENT_DONE` が応答本文の末尾に単独行で現れているか確認する。** 出力に
文字列が含まれているかで判定してはいけない — 依頼文自体にこの文字列が入っているので、
画面には送信した瞬間から自分の依頼のエコーとして現れており、含有チェックは
実装が始まる前から通ってしまう。見るのは「依頼文のエコーより後ろに、報告の
最後の行として単独で出ているか」。
...
- 報告の末尾に単独行の `IMPLEMENT_DONE` が無い出力を最終報告として扱わない
