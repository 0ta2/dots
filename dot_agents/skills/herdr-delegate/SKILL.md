...
- 最後に何を報告してほしいか (下記)
- 報告の最後に単独行で `IMPLEMENT_DONE` と出力すること
...
**`IMPLEMENT_DONE` マーカーが出力に含まれているか確認する。** 含まれていなければ
`--wait` が戻っていても idle 表示が出ていても相手のターンが終わったとみなさない。
数秒待って再度 `herdr agent read` するか `herdr agent wait implementer --until idle`
をやり直す。何度待ってもマーカーが現れない場合は途中経過として扱い、その旨と
生の出力をユーザーに提示して判断を仰ぐ (勝手に完了として報告しない)。
...
- `IMPLEMENT_DONE` マーカーの無い出力を最終報告として扱わない
