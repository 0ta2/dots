...
ユーザーが「新しい 担当者 を立てて」「sonnet でやり直して」のように、同じ種別のまま
別の担当者・別のモデルを求めたときは、**前任者のタブを閉じてから同じ名前で立て直す**。

```bash
herdr tab close <前任者の tab_id>
```

名前に世代番号を足さないのは、1 実装 = 1 タブ = 1 担当者を崩さないため。閉じずに作ると
同じラベルと agent 名になり `agent_name_taken` で落ちる。モデルは `pane run` の起動時にしか
渡せないので、モデルを変える場合も立て直しになる。

**閉じてよいのは、このセッションで自分が作ったタブだけ。** `herdr tab list` /
`herdr agent list` の応答にタブの作成者は無いので、所有権は自分が `tab create` の戻り値で
控えた `tab_id` でしか証明できない。ラベルが一致しても自分が作った記録が無いタブは、
ユーザーが作ったものかもしれないので閉じない。その場合は状況をユーザーに報告して確認を求める。

交代すると前任者の文脈 (それまでの実装の経緯) は消える。実行前にユーザーへその旨を伝える。
