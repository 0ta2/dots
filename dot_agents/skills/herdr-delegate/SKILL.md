...
**マーカー文字列を依頼文に literal で書いてはいけない。理由:** `herdr agent read` が
返すのは端末のスナップショットで、そこには自分が送った依頼文のエコーも含まれる。
依頼文にマーカーを書くと、送信した瞬間から画面にその文字列が存在し、含有チェックは
作業が始まる前に通ってしまう。`recent-unwrapped` はメッセージ境界を持たないので
「エコーより後ろ」を機械的に切り分けることはできない (エコーが `--lines` の窓から
外れる保証もない)。組み立てさせれば、画面にその語を出せるのは応答だけになり、
単純な含有チェックが成立する。
...
**組み立てさせたマーカーが出力に現れているか確認する。** 依頼文にはその語を
書いていないので、画面に出ていれば応答由来と確定できる。

残る誤検知は「最後に IMPLEMENT_DONE を出力します」のような相手の実況だけ。
マーカーが報告の**最後の行**として単独で出ていることも併せて見る。実況と
区別がつかない状態が続くなら「報告を tmp に md で書き、最後の行にマーカーを
入れて、パスだけ返して」と頼み直す。ファイルの末尾行なら端末を介さずに
判定できる。

条件を満たさなければ `--wait` が戻っていても idle 表示が出ていても相手のターンが
終わったとみなさない。数秒待って再度 `herdr agent read` するか
`herdr agent wait implementer --until idle` をやり直す。何度待ってもマーカーが
現れない場合は途中経過として扱い、その旨と生の出力をユーザーに提示して判断を
仰ぐ (勝手に完了として報告しない)。

マーカーはあるが応答全文が出ないなら同じくファイル出力を頼み直す。最初の依頼文で
ファイル出力を求めるのは避ける (画面から取れるなら不要)。

**マーカーは「相手のターンが終わった」ことしか保証しない。** herdr は画面から
状態を推定しているだけで、バックグラウンド実行中を idle と誤判定する既知の不具合が
あるため、まずマーカーでターンの終了を機械的に確認する。そのうえで実装が
受け入れ条件を満たしたかどうかは `git diff` と依頼文で指定したテストを自分で
確認して判定する。マーカーと状態表示と相手の報告文だけで完了扱いにしない。
